...
import { DollarSign, CreditCard, Landmark, Star, Award, ShieldCheck, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function DashboardPage() {
    ...
    const loanHistoryQuery = useMemoFirebase(() => {
        if (!user || !firestore) return null;
        return query(collection(firestore, "loanApplications"), where("userId", "==", user.uid));
    }, [user, firestore]);
    ...
    const totalPaid = paidLoans.reduce((sum, loan) => sum + loan.loanAmount, 0);
    ...
    return (
        <AppLayout>
            <div className="flex items-center justify-between">
                ...
                <Badge variant={userProfile?.isAdmin ? "destructive" : "secondary"}>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{userProfile?.accountTier || 'Bronze'}</div>
                        <p className="text-xs text-muted-foreground">Based on your loan history</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{userProfile?.creditScore || 0}</div>
                        <p className="text-xs text-muted-foreground">Updated based on loan activity</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">Completed Repayments</CardTitle>
                        <CheckCircle2 className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{paidLoans.length}</div>
                        <p className="text-xs text-muted-foreground">Total loans paid off</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{outstandingLoans.length}</div>
                        <p className="text-xs text-muted-foreground">Loans currently being repaid</p>
                    </CardContent>
                </Card>
                    <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">Across {paidLoans.length} loans</p>
                    </CardContent>
                </Card>
            </div>
            <Card>
